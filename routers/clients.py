...
def read_cliente(cliente_id: int, db: Session = Depends(get_db)):
   db_cliente = db.query(models.CRMCliente).filter(models.CRMCliente.id_cliente == cliente_id).first()
   if not db_cliente:
...
def update_cliente(cliente_id: int, cliente: schemas.ClienteUpdate, db: Session = Depends(get_db)):
   ...
   if not db_cliente:
       raise HTTPException(status_code=404, detail="Cliente not found")
   for var, value in cliente.model_dump(exclude_unset=True).items():

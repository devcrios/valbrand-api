...
def listar_planes(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    planes = db.query(models.ProduccionPlan).offset(skip).limit(limit).all()
    return planes

@router.get("/{id_plan}", response_model=schemas.PlanOut)
def obtener_plan(id_plan: int, db: Session = Depends(get_db)):
    plan = db.query(models.ProduccionPlan).filter(
        models.ProduccionPlan.id == id_plan
    ).first()
...
@router.put("/{id_plan}", response_model=schemas.PlanOut)
def actualizar_plan(
    id_plan: int, 
...
def borrar_plan(id_plan: int, db: Session = Depends(get_db)):
    plan = db.query(models.ProduccionPlan).filter(
        models.ProduccionPlan.id == id_plan
    ).first()
